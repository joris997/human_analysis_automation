        ...
        while cnt < length(xsw_static)
            if xst_static(cnt) == 1     % swing
                ...
                if isempty(cntEnd)
                    ...
                    Talus_zsw = Tzsw(cnt-delta:cntEnd);
                    Talus_dzsw = Tdzsw(cnt-delta:cntEnd);
                    tsw = time(cnt-delta:cntEnd);
                else
                    try
                        Talus_zsw = Tzsw(cnt-delta:cntEnd+deltaPlus);
                        Talus_dzsw = Tdzsw(cnt-delta:cntEnd+deltaPlus);
                        tsw = time(cnt-delta:cntEnd+deltaPlus);
                    catch
                        Talus_zsw = Tzsw(cnt-delta:end);
                        Talus_dzsw = Tdzsw(cnt-delta:end);
                        tsw = time(cnt-delta:end);
                    end
                end
                if cntEnd - cnt < 60
                    cnt = cnt + 1;
        ...
        while cnt < length(xst_static)
            if xsw_static(cnt) == 1     % swing
                ...
                if isempty(cntEnd)
                    ...
                    Talus_zsw = Tzst(cnt-delta:cntEnd);
                    Talus_dzsw = Tdzst(cnt-delta:cntEnd);
                    tsw = time(cnt-delta:cntEnd);
                else
                    try
                        Talus_zsw = Tzst(cnt-delta:cntEnd+deltaPlus);
                        Talus_dzsw = Tdzst(cnt-delta:cntEnd+deltaPlus);
                        tsw = time(cnt-delta:cntEnd+deltaPlus);
                    catch
                        Talus_zsw = Tzst(cnt-delta:end);
                        Talus_dzsw = Tdzst(cnt-delta:end);
                        tsw = time(cnt-delta:end);
                    end
                end
                if cntEnd - cnt < 60 
...
titles = {'Downstep','Overstep','Upstep'};
figure
for i = 1:3
    ...
    plot(linspace(0,1,100),zsw_struct.exp100.zsw_mean{i},'LineWidth',2)
    plot(linspace(0,1,100),zsw_struct.unexp100.zsw_mean{i},'LineWidth',2)
    ylabel('$z_{sw}$ (m)','interpreter','latex')
