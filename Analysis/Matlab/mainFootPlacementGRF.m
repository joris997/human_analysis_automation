...
for i = 1:9   
    ...
    for trial = 1:8     % number of trials
        ...
        if isRightLegVLO
            ...
            Fst_idx = getIdxGRF(dataPi,'ground_force_vy');
            Fst_idx = Fst_idx(1);
            Fsw_idx = getIdxGRF(dataPi,'l_ground_force_vy');
        else
            st_idx = getIdxCoM(dataPi,'toes_l_X');
            sw_idx = getIdxCoM(dataPi,'toes_r_X'); 
            Tst_idx = getIdxCoM(dataPi,'talus_r_Y');
            Tsw_idx = getIdxCoM(dataPi,'talus_l_Y');
            Fst_idx = getIdxGRF(dataPi,'l_ground_force_vy');
            Fsw_idx = getIdxGRF(dataPi,'ground_force_vy');
            Fsw_idx = Fsw_idx(1);
        end
        ...
        Fst = dataRi.valuesGRF(:,Fst_idx);
        Fsw = dataRi.valuesGRF(:,Fsw_idx);
...
%         ylabel('zsw [m]')
        ...
        st1 = find(Fst>50,1);
        sw1 = find(Fsw>50,1);
        st2 = find(Fst(round(length(Fst)/2):end),1) + round(length(Fst)/2);
        
        %% Step lengths
        step0 = Txst(st1) - Txsw(1);
        step1 = Txsw(sw1) - Txst(st1);
        step2 = Txst(st2) - Txsw(sw1);
...
function idx = getIdxGRF(data,name)
    idx = find(contains(data.headersGRF,name));
end

function idx = getIdxGRFMean(data,name)
    idx = find(contains(data.headersGRFMean,name));
end