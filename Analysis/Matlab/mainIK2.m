...
%     title(fields{fn})
    ...
    [xinterp,yinterp,zinterp,tinterp,xsd,ysd,zsd] = interpolateWide(x,y,z,t,fn);
    meanCoM.(fields{fn}).x = xinterp;
    meanCoM.(fields{fn}).y = yinterp;
    meanCoM.(fields{fn}).z = zinterp;
    meanCoM.(fields{fn}).t = tinterp;
    meanCoM.(fields{fn}).xsd = max(xsd);
    meanCoM.(fields{fn}).ysd = max(ysd);
    meanCoM.(fields{fn}).zsd = max(zsd);
...
function [xinterp,yinterp,zinterp,tinterp,xsd,ysd,zsd] = interpolateWide(x,y,z,t,run)
